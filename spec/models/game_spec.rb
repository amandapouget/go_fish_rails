...
describe Game do
  let(:players) { Array.new(Game::MAX_PLAYERS + 1) { build(:player) } }
  let(:deck) { build(:deck, :with_cards)}

  describe 'number of players allowed' do
    it "automatically initializes with at least #{Game::MIN_PLAYERS} players" do
      game = Game.new
      expect(game.players.size).to eq Game::MIN_PLAYERS
    end

    it "can only be initialized with up to #{Game::MAX_PLAYERS} players" do
      (Game::MAX_PLAYERS - Game::MIN_PLAYERS).times { |num_players| expect { Game.new(players: players[0..num_players + 1]) }.to_not raise_exception }
      expect { Game.new(players: players[0..Game::MAX_PLAYERS]) }.to raise_error(ArgumentError)
    end
  end
