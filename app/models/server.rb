...
class Server
  ...
  def run(client) # NOT TESTED
    add_user(client)
    sleep 1 # gives it one second to allow more players to connect; so game defaults to Game::MIN_PLAYERS users but could welcome up to Game::MAX_PLAYERS if connect volume is high enough...
    if enough_players?
      users_to_play = []
      Game::MAX_PLAYERS.times { users_to_play << @pending_users.shift unless @pending_users.empty? }
  ...
  def enough_players?
    @pending_users.size >= Game::MIN_PLAYERS
  end
